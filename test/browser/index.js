...
describe('Git-Spy-POC', function(){
  ...
  describe('Registering Patterns', function(){
    before(function(){
      spy.on( patterns.one, callbacks.one );
      spy.on( patterns.two, callbacks.two );
      spy.on( patterns.three, callbacks.three );
      console.log(subscriptions)
    });

    it('patterns should have 4 registrations', function(){
      assert.equal( Object.keys(subscriptions).length, 4 );
    });
